...
public class MySQLStorageProvider extends AbstractStorageProvider {
    
    private HikariDataSource dataSource;
    
    public MySQLStorageProvider(FileConfiguration config) {
        super(config);
    }
    ...
    public void initialize() throws Exception {
        ...
        String database = config.getString("storage.mysql.database", "VanguardClans");
        ...
        hikariConfig.setJdbcUrl("jdbc:mysql://" + host + ":" + port + "/" + database + "?useSSL=false&autoReconnect=true&useUnicode=true&characterEncoding=utf8");
        hikariConfig.setUsername(user);
        hikariConfig.setPassword(password);
        hikariConfig.setDriverClassName("me.lewisainsworth.shaded.mysql.cj.jdbc.Driver");
    ...
    // Implementation of abstract methods (same as H2StorageProvider)
    @Override
    ...
    protected List<String> getClanInvitesImpl(String clanName) throws Exception {
        ...
        long cutoff = getInviteCutoff();
        try (Connection con = getConnection();
             PreparedStatement ps = con.prepareStatement("SELECT username FROM clan_invites WHERE clan = ? AND timestamp >= ?")) {
            ps.setString(1, clanName);
            ps.setLong(2, cutoff);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
