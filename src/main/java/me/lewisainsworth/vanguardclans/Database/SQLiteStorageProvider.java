...
public class SQLiteStorageProvider extends AbstractStorageProvider {
    
    private HikariDataSource dataSource;
    
    public SQLiteStorageProvider(FileConfiguration config) {
        super(config);
    }
        ...
        try (Connection con = getConnection(); Statement stmt = con.createStatement()) {
            stmt.executeUpdate("""
                CREATE TABLE IF NOT EXISTS clans (
                    ...
                    slot_upgrades INTEGER DEFAULT 0
                )
            """);
    ...
    public void addClanPoints(String clanName, int delta) {
        try (Connection con = getConnection();
             PreparedStatement ps = con.prepareStatement("UPDATE clans SET points = MAX(points + ?, 0) WHERE name = ?")) {
            ps.setInt(1, delta);
            ps.setString(2, clanName);
            ps.executeUpdate();
        } catch (SQLException e) {
    ...
    // Implementation of abstract methods (same as other providers but with SQLite-specific syntax)
    @Override
    ...
    protected void setClanHomeImpl(String clanName, Location location) throws Exception {
        try (Connection con = getConnection();
             PreparedStatement ps = con.prepareStatement(
                 "INSERT OR REPLACE INTO clan_homes (clan, world, x, y, z, yaw, pitch) VALUES (?, ?, ?, ?, ?, ?, ?)")) {
            ps.setString(1, clanName);
    ...
    protected void createAllianceImpl(String clan1, String clan2, boolean friendlyFire) throws Exception {
        try (Connection con = getConnection();
             PreparedStatement ps = con.prepareStatement("INSERT OR REPLACE INTO alliances (clan1, clan2, friendly_fire) VALUES (?, ?, ?)")) {
            ps.setString(1, clan1);
            ps.setString(2, clan2);
            ps.setInt(3, friendlyFire ? 1 : 0);
            ps.executeUpdate();
        }
    }
    ...
    protected void setFriendlyFireImpl(String clanName, boolean enabled) throws Exception {
        try (Connection con = getConnection();
             PreparedStatement ps = con.prepareStatement("INSERT OR REPLACE INTO friendlyfire (clan, enabled) VALUES (?, ?)")) {
            ps.setString(1, clanName);
    ...
    protected void addClanInviteImpl(String clanName, String playerName) throws Exception {
        try (Connection con = getConnection();
             PreparedStatement ps = con.prepareStatement("INSERT OR REPLACE INTO clan_invites (clan, username, timestamp) VALUES (?, ?, ?)")) {
            ps.setString(1, clanName);
            ps.setString(2, playerName);
    ...
    protected void addPendingAllianceImpl(String clan1, String clan2) throws Exception {
        try (Connection con = getConnection();
             PreparedStatement ps = con.prepareStatement("INSERT OR REPLACE INTO pending_alliances (clan1, clan2, timestamp) VALUES (?, ?, ?)")) {
            ps.setString(1, clan1);
            ps.setString(2, clan2);
            ps.setLong(3, System.currentTimeMillis());
            ps.executeUpdate();
